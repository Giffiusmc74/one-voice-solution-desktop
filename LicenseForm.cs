    ...
    public partial class LicenseForm : Form
    {
        ...
        private void LicenseForm_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                // ── LOCAL DEV BYPASS ──────────────────────────────────────────
                // When LocalDevMode=true in App.config, skip all license validation
                // and launch MainFormV5 directly. Never ship a build with this enabled.
                string devMode = System.Configuration.ConfigurationManager.AppSettings["LocalDevMode"];
                if (string.Equals(devMode, "true", StringComparison.OrdinalIgnoreCase))
                {
                    logger.Info("[License] LocalDevMode=true — skipping license check for local testing.");
                    if (System.Threading.Interlocked.CompareExchange(ref _isLaunching, 1, 0) == 0)
                    {
                        this.Hide();
                        var devForm = new MainFormV5();
                        devForm.ShowDialog();
                        this.Close();
                    }
                    return;
